...
vuint32_t dspiTstRe = 0; /* 燃油芯片测试结果 */
vuint32_t dspiTstRe1 = 0; /* 电源芯片测试结果 */
...
void dspiBInit(void) 
{
	...
	/* 备用片选时钟与传输属性设置 */
	DSPI_B.CTAR[4].B.DBR = 0; // 0 The baud rate is computed normally with a 50/50 duty cycle
...
void dspi_init (void)
{
	...
	/* 电源芯片dspi通信测试 */
	TXDATA[0] = 0x1002FFB2; /* select pcs1，CTAR[1] */
	count = 0x1;	   
	dspiBTxRv(TXDATA,RXDATA,count);
	if (RXDATA[0] == 0xFFFF || RXDATA[0] == 0)
	{
	    dspiTstRe1 = 1; /* 测试合格 */    
	}
	else
	...
	TXDATA[0] = 0x8001c500;
	TXDATA[1] = 0x800100b8; //00b8
	TXDATA[2] = 0x80010072; //0072
	TXDATA[3] = 0x000100b8; //00b8
	count = 0x4;	   
	dspiBTxRv(TXDATA,RXDATA,count);
	...
	TXDATA[0] = 0x8001eb00;
	TXDATA[1] = 0x8001a285;
	TXDATA[2] = 0x00010632; //0632
	count = 0x3;	   
	dspiBTxRv(TXDATA,RXDATA,count);
	//a5b20
	TXDATA[0] = 0x8001fc00;
	TXDATA[1] = 0x80011f7a; //1f7a
	TXDATA[2] = 0x00011a65; //1a65
	count = 0x3;	   
	dspiBTxRv(TXDATA,RXDATA,count);
...
void sampleKeySwitch(void)
{
    static vuint32_t cnt = 0;
    vuint8_t keySwitch = 0;
    uint32_t data = 0; 
    int32_t res = 0;
    
    keySwitch = fs_gpio_read_data(202);  /* 钥匙开关信号 EMIOS23 */
    ...
    if (0 == keySwitch)
    {
        ...
        if (cnt >= 50)
        {
            data = 0x10020000; /* select pcs1，CTAR[1] */
            res = dspiDataSend(&data, 1);
            //res = dspiDataSend(&data, 1);
           // res = dspiDataSend(&data, 1);
        }
...
void dspiTst(void) 
{
    ...
    delay(5);
    CANATransmitMsg(1, 0x81, (uint8_t *)(&dspiTstRe1), 4);  
    delay(5); /* 延迟5msCANB发送数据 */
    CANBTransmitMsg(1, 0x81, (uint8_t *)(&dspiTstRe1), 4);		
}
